...
export const GlobalContextProvider =({children}) =>{
    ...
    useEffect(()=> {
        ...
        if(battlegroundFromLocalStorage){
            setBattleGround(battlegroundFromLocalStorage)}else{
            localStorage.setItem('battleground', battleGround)
        }}, [])

// Set current wallet to the state
...
// Set the smart contract the provider to the state
  ...
  useEffect(() => {
    ...
    useEffect(()=>{
        if(errorMessage){
            const parsedErrorMessage = errorMessage?.reason?.slice('exectution reverted:'
            .length).slice(0, -1)
    ...
    useEffect(()=>{
        const fetchGameData = async()=>{
            ...
            fetchedBattles.forEach((battle)=>{
                if(battle.players.find((player) => player.toLowerCase()
